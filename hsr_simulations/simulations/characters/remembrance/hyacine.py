...
class Hyacine(Character):
    ...
    def calculate_increased_outgoing_healing_by_spd(self) -> dict[str, list[str | float]]:
        ...
        speed_after_minor_traces = self.speed + self.MINOR_TRACES_SPEED
        ...
        for speed in range(self.speed, self.MAX_SPEED + 1):
            ...
            data_dict["speed_after_minor_traces"].append(speed_after_minor_traces)
            data_dict["speed_after_signature_lightcone"].append(speed_after_minor_traces * (1 + self.SIGNATURE_LIGHTCONE_SPEED_MULT))
            data_dict["speed_after_relics_and_planetary_sets"].append(speed_after_minor_traces * (1 + self.RELICS_SPEED_MULT) * (1 + self.PLANETARY_SPEED_MULT))
